...
class Subspace (object):
    def __init__(self, prefixTuple, rawPrefix=b""):
...
class Queue:
    ...
    def pop(self, db):
        """Pop the next item from the queue. Cannot be composed with other
    ...
    # Items are pushed on the queue at an (index, randomID) pair. Items pushed
    # at the same time will have the same index, and so their ordering will be
    # random. This makes pushes fast and usually conflict free (unless the
    # queue becomes empty during the push)
    ...
    # clients are trying to pop simultaneously, only one will be able to
    # succeed at a time.
    @fdb.transactional
    ...
    # It then enters a polling loop where it attempts to fulfill outstanding
    # pops and then checks to see if it has been fulfilled.
    def _popHighContention(self, db):
        ...
        # fulfilled
        resultKey = self._conflictedItemKey(
            self._conflictedPop.unpack(waitKey)[1])
        ...
        while 1:
            try:
                ...
                # If the error is 1020 (not_committed), then there is a good
                # chance that somebody else has managed to fulfill some
                # outstanding pops. In that case, we proceed to check whether
                # our request has been fulfilled. Otherwise, we handle the
                # error in the usual fashion.
                if e.code != 1020:
...
def queue_multi_client_example(db):
    ...
    for highContention in range(2):
        ...
        pushThreads = [threading.Thread(target=push_thread, args=(
            queue, db, i, 100)) for i in range(10)]
        popThreads = [threading.Thread(target=pop_thread, args=(
            queue, db, i, 100)) for i in range(10)]
        ...
        print('Finished %s in %f seconds' %
              (descriptions[highContention], end - start))
